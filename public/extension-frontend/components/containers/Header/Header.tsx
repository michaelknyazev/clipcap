...
import type { THeader, THeaderFactsState } from './types';
import type { TFacts } from '@clipcap/types';

export const Header = ({ noactions = false }: THeader) => {
  ...
  const handleLoadFacts = async (): Promise<TFacts> => {
    try {
      const { success, result, event } = await FactsService.GetCurrent(
        GetAccessToken()
      );
    ...
    <div className={styles.container}>
      ...
      {!noactions && (
        <div className={`${styles.section} ${styles['section-actions']}`}>
          <ControlGroup>
            <Tooltip content={`Осталось ${left} бесплатных моментов`}>
              <Button
